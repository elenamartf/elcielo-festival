...
function procesarCompra(e) {
    ...
    if (nombre.length < 3) {
        document.getElementById('error-nombre').textContent = 'Por favor, introduce tu nombre completo.';
        valido = false;
    }
    ...
    if (!telRegex.test(telefono.replace(/\s/g, ''))) {
        document.getElementById('error-telefono').textContent = 'Introduce un teléfono válido (9-15 dígitos).';
        valido = false;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ['general', 'vip'].forEach(tipo => {
        ...
        abono.querySelector('.btn-menos').addEventListener('click', () => cambiarCantidad(tipo, -1));
        abono.querySelector('.btn-mas').addEventListener('click', () => cambiarCantidad(tipo, 1));
        abono.querySelector('button.btn-comprar').addEventListener('click', () => abrirModal(tipo));
    });
